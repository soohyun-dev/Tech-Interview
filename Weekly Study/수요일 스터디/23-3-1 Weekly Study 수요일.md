...
9. 이때, 렌더링 엔진이 style 태그를 만난다면 CSS 파싱 작업도 같이 시작하여 CSSOM Tree를 생성.
10. 그러다가 scirpt 태그를 만나면 파싱을 중지하고 자바스크립트 엔진에게 제어 권한을 넘김
11. JS엔진은 코드를 해석하고 추상구문 Tree 인 AST(Abstract Syntax Tree)를 생성하고 실행한다.
12. 중단했던 HTML 파싱을 완료함.
13. DOM Tree + CSSOM Tree = Render Tree // **여기까지가 Construction**
14. 렌더 트리를 기반으로 HTML 요소의 레이아웃(위치와 크기)을 계산하고 브라우저 화면에 HTML 요소를 페인팅한다.
15. UI Backend가 Render Tree의 노드들을 돌면서 UI 를 그림
16. 노드들의 레이어를 순서대로 구성하는 **Composition 단계** **layout~composition = Operation**
17. 최종적으로 사용자에게 화면 출력
...
실행 컨텍스트가 생성될 때마다 this의 바인딩이 일어나며, 우선순위는 다음과 같습니다.
...
4. strict mode(엄격 모드): undefined 로 초기화됩니다.

5. 일반: 브라우저라면 window 객체에 바인딩됩니다.
...
웹 사이트에서 쿠키를 설정하면 모든 웹 요청에는 쿠키 정보가 포함됩니다. => 서버 부하 증가
...
자바스크립트는 싱글 스레드이면서 논 블록킹 언어 입니다.

싱글 스레드이지만 비동기처리를 통해 여러 다른 작업을 동시에 처리하는 것과 같이 보이게 만들어 마치 멀티스레드와 같은 언어처럼 보이게합니다.
