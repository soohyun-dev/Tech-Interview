...
e.stopPropagation 메서드는 이벤트 전파를 중지시킵니다. 이벤트 객체의 경우 상위 태그에도 같은 이벤트가 존재한다면 상위 태그의 해당 콜백함수를 호출하는 특징이 있기 떄문입니다.
...
2. 현재 실행중인 태스크가 종료되지 않은 상태라 해도 다음 태스크를 곧바로 실행하는 방식을 비동기 처리라고 한다.
...
- 블로킹은 작업이 끝날때까지 제어권을 가지고 있는 것을 말한다.
- 논블로킹은 작업이 끝나지 않더라도 서버응답이 나오지 않는다면 제어권을 다음 작업으로 넘겨주는 것을 의미한다.

<br>

## 16. JSON 이 뭔가요?

JavaScript Object Notation의 약자입니다. JSON은 클라이언트와 서버 간의 HTTP 통신을 위한 텍스트 데이터 포맷입니다.
